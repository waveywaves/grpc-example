...
func main() {
	// get configuration
	address := flag.String("server", "http://shiftnode1:30293", "HTTP gateway url, e.g. http://localhost:8080")
	flag.Parse()
	...
	resp, err = http.Get(fmt.Sprintf("%s%s/%s", *address, "/v2/todo", created.ID))
	if err != nil {
		log.Fatalf("failed to call Read method: %v", err)
	}
	bodyBytes, err = ioutil.ReadAll(resp.Body)
	...
	log.Printf("Read response: Code=%d, Body=%s %s\n\n", resp.StatusCode, body, resp)
	...
	// 			"reminder":"%s",
	// 			"work": {
	// 				"description": "bujikfndala",
	// 				"conscription" : "2adsdasd",
	// 				"prescription" : "3asdasd"
	// 			}
	// 		}
	// 	}
	// `, pfx, pfx, pfx)))
